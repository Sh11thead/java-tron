package org.tron.core.db.index;
...
import org.tron.core.db.index.Index.Iface;
...
public abstract class AbstractIndex<E extends ProtoCapsule<T>, T> implements Iface<T> {
  ...
  public AbstractIndex(ITronChainBase<E> database) {
    ...
    String outputDirectory = database.getOutputDirectoryByDbName();
    String indexDirectory = database.getIndexDirectory();

    if (Objects.isNull(parent)){
      parent = new File(outputDirectory + "index");
    }

    File parentDir = Paths.get(outputDirectory, indexDirectory).toFile();
    if (!parentDir.exists()) {
      parentDir.mkdirs();
    }
